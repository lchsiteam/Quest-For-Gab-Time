export function bookEnemy(that,x,y,speed = -1) {  //passes in the this object from phaser as that
    ...
    book.interval = setInterval(() => {
        ...
        if (book.deathClock == 0) {
          ...
          console.log(that.PASSING_OBJ.PATHFINDING.aStarPathfinding(that.currentObstacleCSV,[Math.floor((book.x/32)),Math.floor((book.y/32))],[Math.floor((that.player.x/32)),Math.floor((that.player.x/32))]));
          book.death();
        }
        try {
